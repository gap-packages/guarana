...
GUARANA.EvaluateLieBracket_Symbolic := function( list_x, list_y, scTable )
    ...
    if Length( list_x[1]  ) = 1 and Length( list_y[1] )=1 then 
        ...
        sum1 := GUARANA.EvaluateLieBracket_Symbolic( list_x1, list_y, scTable );
        sum2 := GUARANA.EvaluateLieBracket_Symbolic( list_x2, list_y, scTable );
        ...
        sum1 := GUARANA.EvaluateLieBracket_Symbolic( list_x, list_y1, scTable );
        sum2 := GUARANA.EvaluateLieBracket_Symbolic( list_x, list_y2, scTable );
...
# x corresponds to 1
# y corresponds to 2 
GUARANA.EvaluateLongLieBracket_Symbolic := function( list_x, list_y, com, scTable )
    local r,l,tmp,i;
    tmp := [list_x,list_y];
    r := tmp[com[1]];
...
#
# Example:
# exams_F2c := GUARANA.Get_FNG_TGroupRecords( 2, 9 );;
# recLieAlgs_bch_F2c := List( [2..Length( exams_F2c )], x-> GUARANA.LieAlgebraByTGroupRec( recBCH9,exams_F2c[x] ));;
# recLieAlg := recLieAlgs_bch_F2c[4];
#  GUARANA.AddStarPolynomialsToRecLieAlg( recLieAlg, recBCH9 );
#
GUARANA.AddStarPolynomialsToRecLieAlg := function( recLieAlg, recBCH )
...
# 
GUARANA.ComputeSymbolicLogPolynomials := function( recLieAlg, recBCH )
    ...
    for i in Reversed( [1..(n-1)] ) do 
        ...
        tail := GUARANA.ComputeStarPolys( recBCH, x_i, tail,  w_x_i, 
...
GUARANA.ComputeSymbolicLogPolynomialsByStarPols := function( recLieAlg, recBCH )
...
GUARANA.AddStarLogAndExpPols := function( args )
    ...
    GUARANA.AddLogPolynomialsToLieAlgRecord( recLieAlg, recBCH9 ); 
    GUARANA.AddExpPolynomialsToLieAlgRecord( recLieAlg, recBCH9 ); 
    return 0;
end;
